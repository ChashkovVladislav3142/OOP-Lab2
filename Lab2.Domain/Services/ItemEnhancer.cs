...
public class ItemEnhancer
{
    ...
    public IItem Combine(IItem item1, IItem item2)
    {
        if (item1.Name == item2.Name && item1.GetType() == item2.GetType())
        {
            ...
            if (item1 is Weapon w1 && item2 is Weapon w2)
            {
                return builder.SetDamage(w1.Damage + 2).SetStrategy(new EquipStrategy()).BuildWeapon();
            }
            if (item1 is Armor a1 && item2 is Armor a2)
            {
                return builder.SetDefense(a1.Defense + 1).SetStrategy(new EquipStrategy()).BuildArmor();
            }
